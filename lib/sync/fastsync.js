...
class FastSynchronizer extends Synchronizer {
  ...
  async syncWithPeer (peer) {
    ...
    console.log(`Syncing with peer: ${peer.toString(true)} height=${height.toString(10)}`)
    ...
    this.blockFetcher
      ...
      .on('fetched', blocks => {
        ...
        console.log(`Imported blocks count=${blocks.length} number=${first.toString(10)} hash=${hash} peers=${this.pool.size}`)
      })
  ...
  async announced (announcements, peer) {
    if (announcements.length) {
      ...
      console.log(`New height: number=${height.toString(10)} hash=${short(hash)}`)
  ...
  async open () {
    ...
    console.log(`Latest local block: number=${number} td=${td} hash=${short(hash)}`)
  }
