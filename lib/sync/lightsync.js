...
class LightSynchronizer extends Synchronizer {
  ...
  async syncWithPeer (peer) {
    ...
    this.headerFetcher
      ...
      .on('fetched', headers => {
        ...
        console.log(`Imported headers count=${headers.length} number=${first.toString(10)} hash=${hash} peers=${this.pool.size}`)
      })
  ...
  async open () {
    ...
    console.log(`Latest local header: number=${number} td=${td} hash=${short(hash)}`)
  }
