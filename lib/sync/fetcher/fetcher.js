...
class Fetcher extends Readable {
  ...
  expire (job) {
    ...
    if (this.pool.contains(job.peer)) {
      console.log(`Task timed out for peer (banning) ${JSON.stringify(job.task)} ${job.peer}`);
      this.pool.ban(job.peer, 300000);
    } else {
      console.log(`Peer disconnected while performing task ${JSON.stringify(job.task)} ${job.peer}`);
    }
    this.enqueue(job);
  }
